...
## 1. The 100% gate omits three domain modules and both core service packages

`.coveragerc` runs to 93 indented lines of `omit`, and what it omits has grown well past UI and I/O:

**Domain modules** (pure, stdlib-only, no I/O, no reason to be hard to test):
...
**Application service packages**, both entire:
...
The stated reason is "not practical to exhaustively unit test (hardware/threads/Qt event loop)" and "heavily heuristic/regex-driven and covered via higher-level service tests". For the audio and TTS modules that is exactly right and they belong in the section below. It does not hold for these five entries.

The domain layer is the one place in this repository where the reason cannot apply: there is no hardware, no thread and no event loop in `chunking_service.py`. And the structural-bookmark package is demonstrably testable, because `tests/application/test_structural_bookmark_service.py`, `test_structural_bookmark_service_axioms.py` and `test_structural_bookmark_duplicates.py` already test it. It is tested and then excluded from the gate, which means the gate cannot tell anyone whether that testing is still complete.

The result is that NarrateX's headline claim (a 100% gate) is true of the modules that were easy and silent about the modules that decide how a book is actually chunked, aligned and sanitised for speech. That is the inverse of what a gate is for.

The proportionate fix, in order:

1. Remove the three domain modules from `omit` and close whatever gap appears. Domain is pure; this should be the smallest piece of work in this file.
2. Remove `structural_bookmarks/*` and `structural_bookmark_service.py` and let the existing tests carry as much as they carry, then close the remainder.
3. Leave `narration/*` omitted, and say so explicitly with the thread and audio-device justification, rather than leaving it inside a general-purpose list.
...
None of this is wrong, and all of it is the file telling you it has been edited under pressure. The backslash duplication in particular suggests the underlying problem was fought rather than solved: coverage path normalisation is configurable, and one `[paths]` section would remove every backslash variant in the file.

Fixing item 1 means editing this list anyway. Do the tidy in the same pass.

## 3. There is no `VERSION` file and the published site is not stamped

`voice_reader/version.py` holds `__version__: str = "4.0.0"` as a literal, and `pyproject.toml` reads it through `version = { attr = "voice_reader.version.__version__" }`. That single-import chain is good and better than most.

What is missing is the file at the top of it. There is no `VERSION`, no `stamp_version.py` and no delimited tokens in the markdown or in the site pages. The site is `index.html`, `why.html` and `download.html` at repository root, and every version-bearing string on those pages (download links, screenshots, release copy) is edited by hand. The commit history shows exactly that: "4.0.0 screenshot everywhere and a current sitemap date".

Adopt the pattern: `VERSION` at root, `version.py` reading it with a `0.0.0-dev` fallback, `stamp_version.py` writing the tokens into the root markdown and the three site pages, and `buildexe.py` and `buildinstaller.py` calling it at the start of every build. That converts a remembered manual sweep into a build rule.

`version.py` also carries `APP_COPYRIGHT` with a year in it. Drop the year; it goes stale on a fixed date for no benefit.

## 4. The published site shares a directory with the application source
...
Nothing is broken by this and GitHub Pages supports it. The cost is that root has 41 tracked entries where the substance is one package and five build scripts, and that no tool can distinguish "the site" from "the app" when reasoning about either. It also makes item 3's stamping step harder to scope than it needs to be.

Moving the site into `docs/` and pointing Pages at it is mechanical, and worth doing at the same time as item 3 since both touch the same files.

## 5. Ten em dashes in prose, in code that ships

The em-dash ban is absolute across this repository. Ten live in prose:

- `builddmg.py`: eight, in the module docstring's environment-variable table and in five `print` strings the developer sees during a macOS build.
- `voice_reader/infrastructure/audio/sounddevice_streamer.py:86-87`: two, in a comment explaining the `afplay` fallback.

These are the only prose instances. Everything else the scan finds is data and is covered in the last section below. Replace with a colon, a comma or parentheses.

## 6. The 5% danger band is documented and not enforced
...
## 7. Broad exception handlers on the startup path and in the installer
...
- `builddmg.py` at 577 lines. Delivery script, exempt from the cap by design and correctly listed in `_BUILD_SCRIPTS` in the LOC test. Item 5 concerns its prose, not its length.
...
- **The separate `installer/ops` and `installer/ui` packages with their own test package.** Correct decomposition; item 7 is about the handlers inside it, not the shape.
