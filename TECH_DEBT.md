...
- **The chapter heading that reads `CHAPTER 2 \` in *Saving Faith*.** One occurrence in 767 headings measured across 24 books; the backslash is in the book's own source text. Trimming it would clean only the spine's label while the page still showed it, because blocks are a view over the source rather than a copy. Leave it unless a new measurement shows the shape recurring.
